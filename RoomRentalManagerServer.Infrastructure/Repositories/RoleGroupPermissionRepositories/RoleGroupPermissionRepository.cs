...
namespace RoomRentalManagerServer.Infrastructure.Repositories.RoleGroupPermissionRepositories
{
    public class RoleGroupPermissionRepository : IRoleGroupPermissionRepository
    {
        ...
        public async Task<RoleGroupPermission> GetByIdAsync(long id)
        {
            try
            {
                return await _context.RoleGroupPermission.AsNoTracking().FirstOrDefaultAsync(x => x.Id == id);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IQueryable<RoleGroupPermission>> GetByRoleGroupIdAsync(long roleGroupId)
        {
            ...
            {
                _logger.LogError($"Failed to get RoleGroupPermission by roleGroupId: {ex.Message}");
                throw;
            }
        }
    }
